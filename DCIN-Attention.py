...
x_input = tf.compat.v1.placeholder(tf.int32, shape=[None, total_num_ft_col])

x_input_one_hot, x_input_mul_hot, x_input_one_hot_aux, x_input_mul_hot_aux \
    = partition_input(x_input)

# target vect
y_target = tf.compat.v1.placeholder(tf.float32, shape=[None, 1])
# dropout keep prob
...
for i in range(num_aux_type):
    ...
    W1_list[i] = tf.Variable(tf.random.normal([total_embed_dim[i], att_hidden_dim],
                                              stddev=std_a))
    b1_list[i] = tf.Variable(tf.random.normal([att_hidden_dim], stddev=std_b))
    W2_list[i] = tf.Variable(tf.random.normal([att_hidden_dim, 1], stddev=std_b))
    b2_list[i] = tf.Variable(tf.random.normal([1], stddev=0.01))
...
field_size = data_embed.get_shape().as_list()[-1] // k
y_deep = tf.reshape(data_embed, [-1, field_size, k])
...
flat = tf.reshape(y_deep, shape=[-1, output_size * field_size])
input_size = output_size * field_size
weights = dict()
# ---------- prediciton weight ------------------#
glorot = np.sqrt(2.0 / (input_size + 1))
weights["prediction"] = tf.Variable(np.random.normal(loc=0, scale=glorot, size=(input_size, 1)), dtype=np.float32,
                                    name="prediction")
weights["prediction_bias"] = tf.Variable(np.random.normal(), dtype=np.float32, name="prediction_bias")
out = tf.add(tf.matmul(flat, weights["prediction"]), weights["prediction_bias"])  # None * 1
...
if opt_alg == 'Adam':
    optimizer = tf.compat.v1.train.AdamOptimizer(eta).minimize(loss)
else:
    # default
    optimizer = tf.compat.v1.train.AdagradOptimizer(eta).minimize(loss)
